...
rem Force UTF-8 for the Maven JVM so the demo output stays consistent with the console.
...
echo ============================================================
echo  Service discovery demo (quick start RegistryResolver)
...
rem Install the core SDK into the local repository (needed once, safe to repeat).
call mvn -q -DskipTests -f "..\..\..\java\pom.xml" -pl waymark-java -am install

call mvn -q exec:java -Dexec.mainClass=com.wueasy.waymark.examples.DiscoveryDemo %*
